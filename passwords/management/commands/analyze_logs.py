...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f"Error analyzing logs: {str(e)}")
            )
    ...
    def parse_log_entry(self, line, start_date, end_date):
        ...
        timestamp_str, level, logger_name, module, function, line_num, message = match.groups()
    ...
    def detect_suspicious_activities(self, entry, results):
        ...
        if 'login failed' in message:
            ...
            if ip_match:
                ...
                recent_failures = [
                    f for f in results['failed_logins']
                    if f['ip'] == ip and
                       (entry['timestamp'] - f['timestamp']).total_seconds() < 300  # 5 minutes
                ]
        ...
        if 'password' in message and ('added' in message or 'updated' in message):
            ...
            if user_match:
                username = user_match.group(1)
                recent_ops = [
                    entry['timestamp'] for entry in results.get('recent_password_ops', [])
                    if entry.get('user') == username and
                       (entry['timestamp'] - entry['timestamp']).total_seconds() < 60
                ]
    ...
    def generate_alerts(self, results, alert_threshold):
        ...
        off_hours_admin = [
            activity for activity in results['admin_activities']
    ...
    def print_text_report(self, results):
        ...
        if results['failed_logins']:
            ...
            for failure in results['failed_logins'][-10:]:  # Show last 10
                self.stdout.write(f"  {failure['timestamp']} - User: {failure['username']}, IP: {failure['ip']}")
            self.stdout.write("")
        ...
        if results['suspicious_activities']:
            ...
            for activity in results['suspicious_activities']:
                self.stdout.write(f"  {activity['timestamp']} - Type: {activity['type']}")
            self.stdout.write("")
        ...
        if results['alerts']:
            ...
            for alert in results['alerts']:
                style = self.style.ERROR if alert['type'] == 'CRITICAL' else self.style.WARNING
...
def security_alert_callback(log_entry):
    ...
    if any(keyword in log_entry.lower() for keyword in ['failed', 'error', 'warning', 'suspicious']):
    ...
    for filename in os.listdir(logs_dir):
        if filename.endswith('.log'):
            ...
            if file_size > 50 * 1024 * 1024:
                logger.warning(f"Large log file detected: {filename} ({file_size / 1024 / 1024:.1f} MB)")