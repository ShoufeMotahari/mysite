...
class Section(models.Model):
    ...
    slug = models.SlugField(
        max_length=250, unique=True, blank=True, verbose_name=_("Slug")
    )
    ...
    def breadcrumb_path(self):
        ...
        return " / ".join(
            [ancestor.title for ancestor in reversed(self.get_ancestors())]
            + [self.title]
        )
    ...
    def get_siblings(self):
        ...
        return Section.objects.filter(parent=self.parent, level=self.level).exclude(
            pk=self.pk
        )
    ...
    def get_next_sibling(self):
        ...
        return (
            self.get_siblings().filter(order__gt=self.order).order_by("order").first()
        )

    def get_previous_sibling(self):
        """Get the previous sibling by order"""
        return (
            self.get_siblings().filter(order__lt=self.order).order_by("-order").first()
        )
    ...
    def move_to_position(self, new_order):
        ...
        siblings = Section.objects.filter(parent=self.parent, level=self.level).exclude(
            pk=self.pk
        )
                ...
                tree.append(
                    {
                        "section": section,
                        "children": build_tree(
                            section.children.filter(is_active=True).order_by("order")
                        ),
                    }
                )
            return tree
    ...
    def display_title(self):
        ...
        if self.level == 1:
            ...
            grandparent_order = (
                self.parent.parent.order if self.parent and self.parent.parent else 1
            )
            parent_order = self.parent.order if self.parent else 1
