...
def section_tree(request):
    ...
    try:
        ...
        return JsonResponse({
            'success': True,
            'tree': serialize_tree(tree_structure)
        })
        ...
        for section in sections:
            section_data = {
                ...
                'children': build_admin_tree(
                    section.children.all().order_by('order'),
                    parent_level + 1
                )
            }
            result.append(section_data)
...
def admin_sections_bulk_action(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if action == 'activate':
                ...
                for section in sections:
                    if section.children.exists():
                        return JsonResponse({
                            'success': False,
                            'error': _(f'Cannot delete "{section.title}" - it has child sections')
                        }, status=400)
            ...
            logger.info(f'Bulk action "{action}" performed on {len(section_ids)} sections')
...
def section_navigation(request):
    ...
    try:
        ...
        def create_nav_structure(tree):
            ...
            for item in tree:
                nav_item = {
                    ...
                    'children': create_nav_structure(item['children']) if item['children'] else []
                }
        ...
        return JsonResponse({
            'success': True,
            'navigation': create_nav_structure(tree_structure)
        })
