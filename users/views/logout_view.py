...
def smart_logout_view(request):
    ...
    if request.user.is_superuser:
        ...
        logout_message = 'مدیر ارشد با موفقیت خارج شد.'
          ...
          request.user.user_type.slug == 'message_admin'):
...
class SmartLogoutView(LogoutView):
    ...
    def get_next_page(self):
        ...
        if self.request.user.is_superuser:
            return '/admin/login/'
        elif (self.request.user.is_staff and
              hasattr(self.request.user, 'user_type') and
              self.request.user.user_type and
              self.request.user.user_type.slug == 'message_admin'):
            return '/admin/login/'
