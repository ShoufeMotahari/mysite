...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--create-defaults',
            action='store_true',
            help='Create default user types'
        )

        parser.add_argument(
            '--list',
            action='store_true',
            help='List all user types'
        )

        parser.add_argument(
            '--stats',
            action='store_true',
            help='Show user type statistics'
        )
        ...
        parser.add_argument(
            '--to-users',
            type=str,
            help='Comma-separated list of usernames/emails'
        )
        ...
        parser.add_argument(
            '--set-default',
            type=str,
            help='Set a user type as default'
        )
    ...
    def list_user_types(self):
        """List all user types"""
        user_types = UserType.objects.annotate(
    ...
    def show_stats(self):
        ...
        # Type distribution
        type_stats = UserType.objects.annotate(
            users_count=Count('user')
        ).filter(users_count__gt=0).order_by('-users_count')
        ...
        if type_stats.exists():
            ...
            for stat in type_stats:
                ...
                self.stdout.write(f'{stat.name}: {stat.users_count} ({percentage:.1f}%)')
    ...
    def assign_type_to_users(self, type_slug, users_list):
        ...
        for user in users:
            ...
            if user_type.can_access_admin:
                user.is_staff = True
            elif old_type and old_type.can_access_admin and not user_type.can_access_admin:
                user.is_staff = False
    ...
    def cleanup_users_without_type(self):
        ...
        if count == 0:
            self.stdout.write(self.style.SUCCESS('All users already have a type assigned.'))
            return
    ...
    def get_user_type_by_slug_or_name(self, identifier):
        ...
        try:
            return UserType.objects.get(
                Q(slug=identifier) | Q(name=identifier),
                is_active=True
            )
        except UserType.DoesNotExist:
            available_types = UserType.objects.filter(is_active=True).values_list('slug', 'name')
